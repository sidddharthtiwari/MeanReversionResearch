...
from src.features.volume_zscore import compute_volume_zscore
...
# --------------------------------------------------
# TEST 1
# Basic Volume Z-Score
# --------------------------------------------------


def test_basic_volume_zscore() -> None:
    df = _make_volume_frame([100.0, 200.0, 300.0, 400.0, 500.0])
    result = compute_volume_zscore(df, window=3)

    assert "volume_zscore_3" in result.columns
...
def test_custom_output_column() -> None:
    ...
    result = compute_volume_zscore(
        df,
        window=2,
        output_column="custom_volume_z",
    )

    assert "custom_volume_z" in result.columns
    assert "volume_zscore_2" not in result.columns
...
def test_group_isolation() -> None:
    ...
    window = 3
    result = compute_volume_zscore(df, window=window)
    column = f"volume_zscore_{window}"

    only_a = compute_volume_zscore(
        df[df["symbol"] == "A"].reset_index(drop=True),
        window=window,
    )
    only_b = compute_volume_zscore(
...
def test_matches_manual_formula() -> None:
    ...
    expected = (df["volume"] - rolling_mean) / rolling_std

    pdt.assert_series_equal(
        result[f"volume_zscore_{window}"],
        expected,
        check_names=False,
    )
...
def test_invalid_window() -> None:
    ...
    try:
        compute_volume_zscore(df, window=0)
        raised = False
...
def test_invalid_min_periods() -> None:
    ...
    try:
        compute_volume_zscore(df, window=3, min_periods=0)
        raised = False
    except ValueError:
        raised = True
...
def test_missing_volume_column() -> None:
    ...
    try:
        compute_volume_zscore(df, window=2)
        raised = False
...
def test_input_dataframe_unchanged() -> None:
    ...
    compute_volume_zscore(df, window=3)
...
def test_output_schema() -> None:
    df = _make_volume_frame([100.0, 200.0, 300.0])
    result = compute_volume_zscore(df, window=2)

    expected_columns = list(df.columns) + ["volume_zscore_2"]
...
def test_custom_volume_column() -> None:
    ...
    rolling_mean = df["trade_volume"].rolling(window=window).mean()
    rolling_std = df["trade_volume"].rolling(window=window).std()
    expected = (df["trade_volume"] - rolling_mean) / rolling_std

    assert f"volume_zscore_{window}" in result.columns
    pdt.assert_series_equal(
        result[f"volume_zscore_{window}"],
        expected,
        check_names=False,
...
def test_default_output_name() -> None:
    df = _make_volume_frame([100.0, 200.0, 300.0, 400.0, 500.0])
    result = compute_volume_zscore(df, window=5, output_column=None)

    assert "volume_zscore_5" in result.columns


def main() -> None:
    test_basic_volume_zscore()
    ...
    print("🎉 ALL VOLUME Z-SCORE TESTS PASSED")
