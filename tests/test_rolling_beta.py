...
from src.features.rolling_beta import compute_rolling_beta
...
def test_basic_rolling_beta() -> None:
    df = _make_pair_frame(
    ...
    assert "rolling_beta_3" in result.columns
...
def test_custom_output_column() -> None:
    ...
    result = compute_rolling_beta(
        ...
        left_column="left",
        right_column="right",
        output_column="custom_beta",
    )

    assert "custom_beta" in result.columns
    assert "rolling_beta_2" not in result.columns
...
def test_group_isolation() -> None:
    ...
    column = f"rolling_beta_{window}"

    only_a = compute_rolling_beta(
        df[df["symbol"] == "A"].reset_index(drop=True),
        window=window,
        left_column="left",
        right_column="right",
    )
    only_b = compute_rolling_beta(
...
def test_matches_manual_formula() -> None:
    ...
    expected = (
        df["left"].rolling(window=window).cov(df["right"])
        / df["right"].rolling(window=window).var()
    )

    pdt.assert_series_equal(
        result[f"rolling_beta_{window}"],
        expected,
        check_names=False,
    )
...
def test_output_schema() -> None:
    ...
    expected_columns = list(df.columns) + ["rolling_beta_2"]
    assert list(result.columns) == expected_columns


def main() -> None:
    test_basic_rolling_beta()
    ...
    print("🎉 ALL ROLLING BETA TESTS PASSED")
