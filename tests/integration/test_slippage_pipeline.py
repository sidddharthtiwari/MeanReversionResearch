...
def test_slippage_pipeline() -> None:
    window = 3
    entry_threshold = 1.0
    slippage = 0.001
    ...
    slippage_column = f"{strategy_return_column}_slippage"
    ...
    # 3. Slippage column exists.
    assert slippage_column in result.columns

    # 4. Net-return column exists.
    assert net_return_column in result.columns

    # 5. All slippage values are non-negative.
    assert (result[slippage_column] >= 0).all()

    # 6. Net returns equal strategy_return - slippage.
    expected_net_returns = (
        result[strategy_return_column] - result[slippage_column]
    )
    ...
    # 7. Generated columns contain no NaN values.
    ...
    # 9. Output length matches input length.
    assert len(result) == len(df)


def main() -> None:
    test_slippage_pipeline()

    print("🎉 SLIPPAGE PIPELINE TEST PASSED")
