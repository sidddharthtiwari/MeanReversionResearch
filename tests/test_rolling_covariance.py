...
# --------------------------------------------------
# TEST 1
# Basic Rolling Covariance
# --------------------------------------------------


def test_basic_rolling_covariance() -> None:
    df = _make_pair_frame(
    ...
    assert "rolling_covariance_3" in result.columns
...
def test_custom_output_column() -> None:
    ...
    result = compute_rolling_covariance(
        ...
        left_column="left",
        right_column="right",
        output_column="custom_cov",
    )

    assert "custom_cov" in result.columns
    assert "rolling_covariance_2" not in result.columns
...
def test_group_isolation() -> None:
    ...
    column = f"rolling_covariance_{window}"

    only_a = compute_rolling_covariance(
        df[df["symbol"] == "A"].reset_index(drop=True),
        window=window,
        left_column="left",
        right_column="right",
    )
    only_b = compute_rolling_covariance(
...
def test_matches_manual_formula() -> None:
    ...
    expected = df["left"].rolling(window=window).cov(df["right"])

    pdt.assert_series_equal(
        result[f"rolling_covariance_{window}"],
        expected,
        check_names=False,
    )
...
def test_output_schema() -> None:
    ...
    expected_columns = list(df.columns) + ["rolling_covariance_2"]
    assert list(result.columns) == expected_columns


def main() -> None:
    test_basic_rolling_covariance()
    ...
    print("🎉 ALL ROLLING COVARIANCE TESTS PASSED")
