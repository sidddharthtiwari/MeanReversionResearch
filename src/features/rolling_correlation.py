"""Per-symbol rolling Pearson correlation for quantitative research.

This module computes rolling correlation between two numeric columns within
...
ROLLING_CORRELATION_PREFIX = "rolling_correlation"
...
def _generate_output_column_name(
    ...
    """Resolve the rolling-correlation output column name.
    ...
    Returns:
        ...
        ``rolling_correlation_{window}``.
    """
    if output_column is not None:
        return output_column
    return f"{ROLLING_CORRELATION_PREFIX}_{window}"
...
def _compute_rolling_correlation(
    ...
    """Append per-symbol rolling Pearson correlation to a copied DataFrame.

    Args:
        df: Input DataFrame containing ``symbol`` and both numeric columns.
        window: Rolling window size.
        left_column: First numeric column in the correlation pair.
        right_column: Second numeric column in the correlation pair.
        output_column: Final resolved name for the correlation column.
        min_periods: Minimum observations required; ``None`` uses ``window``.

    Returns:
        A new DataFrame with the rolling-correlation column appended. The
    ...
    result = df.copy()
    rolling_correlations: list[pd.Series] = []
    for _, symbol_df in _group_by_symbol(result):
        group_corr = (
            symbol_df[left_column]
            .rolling(window=window, min_periods=min_periods)
            .corr(symbol_df[right_column])
        )
        rolling_correlations.append(group_corr)

    rolling_corr = pd.concat(rolling_correlations).sort_index()
    result[output_column] = rolling_corr
    return result
...
def compute_rolling_correlation(
    ...
    Correlation windows never cross symbol boundaries.
    ...
    return _compute_rolling_correlation(
        df=df,
        window=window,
